...
export default function ChatInput({onSend, isLoading}:ChatInputProps){
    ...
    return <div className = {"h-auto w-full flex flex-row gap-[10px] pt-[10px] border-t-stone-300 border-t pt-[20px]"}>
        ...
        <button className= {`border flex items-center justify-center  text-white rounded-full w-[50px] h-12 shrink-0 shadow-md ${isLoading? "bg-stone-400" :"bg-black"}`} onClick = {()=>{if(!isLoading){ onSend(text); setText("")}}}>
            <svg className={"w-7 h-7 ml-[-3px] mb-[-2px]"} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
